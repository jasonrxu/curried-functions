...
///         |> filter { $0 > 1 }
///         |> map { $0 |> map }
public func |><I, O>(input: I, transform: I -> O) -> O {
...
/// is equal to
///     distance(start, end)
public func |><I1, I2, O>(input: (I1, I2), transform: (I1, I2) -> O) -> O {
...
public func |><I1, I2, I3, O>(input: (I1, I2, I3), transform: (I1, I2, I3) -> O) -> O {
...
public func advance<T : ForwardIndexType>(n: T.Distance, end: T)(start: T) -> T {
...
public func advance<T: ForwardIndexType>(n: T.Distance)(start: T) -> T {
    return advance(start, n)
}


/// Return `true` iff an element in `seq` satisfies `predicate`.
public func contains<S : SequenceType, L : BooleanType>(@noescape predicate: (S.Generator.Element) -> L)(seq: S) -> Bool {
    return contains(seq, predicate)
}


/// Return `true` iff `x` is in `seq`.
public func contains<S : SequenceType where S.Generator.Element : Equatable>(x: S.Generator.Element)(seq: S) -> Bool {
...
/// result.
public func distance<T : ForwardIndexType>(end: T)(start: T) -> T.Distance {
...
public func equal<S1 : SequenceType, S2 : SequenceType where S1.Generator.Element == S2.Generator.Element>(a2: S2, @noescape isEquivalent: (S1.Generator.Element, S2.Generator.Element) -> Bool)(a1: S1) -> Bool {
...
/// same order.
public func equal<S1 : SequenceType, S2 : SequenceType where S1.Generator.Element == S2.Generator.Element, S1.Generator.Element : Equatable>(a2: S2)(a1: S1) -> Bool {
    return equal(a1, a2)
}
...
/// O(N)
public func extend<C : RangeReplaceableCollectionType, S : CollectionType where C.Generator.Element == S.Generator.Element>(newElements: S)(inout x: C) {
...
public func find<C : CollectionType where C.Generator.Element : Equatable>(value: C.Generator.Element)(domain: C) -> C.Index? {
...
public func flatMap<S : SequenceType, T>(@noescape transform: (S.Generator.Element) -> [T])(source: S) -> [T] {
    return flatMap(source, transform)
}


/// Returns `f(self)!` iff `self` and `f(self)` are not nil.
public func flatMap<T, U>(@noescape f: (T) -> U?)(x: T?) -> U? {
    return flatMap(x, f)
}
...
public func insert<C : RangeReplaceableCollectionType>(newElement: C.Generator.Element, atIndex i: C.Index)(inout x: C) {
...
public func join<C : ExtensibleCollectionType, S : SequenceType where C == S.Generator.Element>(elements: S)(separator: C) -> C {
...
public func lexicographicalCompare<S1 : SequenceType, S2 : SequenceType where S1.Generator.Element == S2.Generator.Element, S1.Generator.Element : Comparable>(a2: S2)(a1: S1) -> Bool {
...
public func lexicographicalCompare<S1 : SequenceType, S2 : SequenceType where S1.Generator.Element == S2.Generator.Element>(a2: S2, @noescape isOrderedBefore less: (S1.Generator.Element, S1.Generator.Element) -> Bool)(a1: S1) -> Bool {
...
public func max<T : Comparable>(y: T, z: T, rest: T...)(x: T) -> T {
    return maxElement([x, y, z] + rest)
}


/// Return the greater of `x` and `y`
public func max<T : Comparable>(y: T)(x: T) -> T {
    return max(x, y)
}


/// Return the lesser of `x` and `y`
public func min<T : Comparable>(y: T)(x: T) -> T {
    return min(x, y)
}


/// Return the least argument passed
public func min<T : Comparable>(y: T, z: T, rest: T...)(x: T) -> T {
    return minElement([x, y, z] + rest)
}


/// Returns `true` if `lhs` and `rhs` have a non-empty intersection
public func overlaps<I0 : IntervalType, I1 : IntervalType where I0.Bound == I1.Bound>(rhs: I1)(lhs: I0) -> Bool {
...
/// <http://en.wikipedia.org/wiki/Strict_weak_order#Strict_weak_orderings>`__
/// over `elements`.
public func partition<C : MutableCollectionType where C.Index : RandomAccessIndexType>(range: Range<C.Index>, isOrderedBefore: (C.Generator.Element, C.Generator.Element) -> Bool)(inout elements: C) -> C.Index {
...
/// *p*\ `])`.  Only returns `range.endIndex` when `elements` is
/// empty.
/// Requires: The less-than operator (`public func <`) defined in the `Comparable`
/// conformance is a `strict weak ordering
/// <http://en.wikipedia.org/wiki/Strict_weak_order#Strict_weak_orderings>`__
/// over `elements`.
public func partition<C : MutableCollectionType where C.Index : RandomAccessIndexType, C.Generator.Element : Comparable>(range: Range<C.Index>)(inout elements: C) -> C.Index {
...
/// of slicing `s`.
public func prefix<S : Sliceable>(maxLength: Int)(s: S) -> S.SubSlice {
...
@inline(never) public func print<T, TargetStream : OutputStreamType>(inout target: TargetStream)(object: T) {
    print(object, &target)
}
...
/// Do not overload this function for your type.  Instead, adopt one of the
/// protocols mentioned above.
@inline(never) public func println<T, TargetStream : OutputStreamType>(inout target: TargetStream)(object: T) {
...
/// `sequence`, in turn.
public func reduce<S : SequenceType, U>(initial: U, @noescape combine: (U, S.Generator.Element) -> U)(sequence: S) -> U {
...
///
/// Complexity: O(\ `countElements(x)`\ ).
public func removeAll<C : RangeReplaceableCollectionType>(keepCapacity: Bool = false)(inout x: C) {
...
///
/// Complexity: O(\ `countElements(x)`\ ).
public func removeAtIndex<C : RangeReplaceableCollectionType>(index: C.Index)(inout x: C) -> C.Generator.Element {
...
///
/// Complexity: O(\ `countElements(x)`\ ).
public func removeRange<C : RangeReplaceableCollectionType>(subRange: Range<C.Index>)(inout x: C) {
...
/// <http://en.wikipedia.org/wiki/Strict_weak_order#Strict_weak_orderings>`__
/// over `elements`.
public func sort<T>(isOrderedBefore: (T, T) -> Bool)(inout array: ContiguousArray<T>) {
    sort(&array, isOrderedBefore)
}

public func sort<T>(isOrderedBefore: (T, T) -> Bool)(inout array: [T]) {
    sort(&array, isOrderedBefore)
}
...
/// <http://en.wikipedia.org/wiki/Strict_weak_order#Strict_weak_orderings>`__
/// over `elements`.
public func sort<C : MutableCollectionType where C.Index : RandomAccessIndexType>(isOrderedBefore: (C.Generator.Element, C.Generator.Element) -> Bool)(inout collection: C) {
...
public func splice<C : RangeReplaceableCollectionType, S : CollectionType where C.Generator.Element == S.Generator.Element>(newElements: S, atIndex i: C.Index)(inout x: C) {
...
public func startsWith<S0 : SequenceType, S1 : SequenceType where S0.Generator.Element == S1.Generator.Element, S0.Generator.Element : Equatable>(prefix: S1)(s: S0) -> Bool {
...
/// <http://en.wikipedia.org/wiki/Equivalence_relation>`_
public func startsWith<S0 : SequenceType, S1 : SequenceType where S0.Generator.Element == S1.Generator.Element>(prefix: S1, @noescape isEquivalent: (S0.Generator.Element, S1.Generator.Element) -> Bool)(s: S0) -> Bool {
...
///      sequence.
public func stride<T : Strideable>(through end: T, by s: T.Stride)(from start: T) -> StrideThrough<T> {
...
public func stride<T : Strideable>(to end: T, by s: T.Stride)(from start: T) -> StrideTo<T> {
...
/// `RandomAccessIndexType` and O(N)+K otherwise, where K is the cost
/// of slicing `s`.
public func suffix<S : Sliceable where S.Index : BidirectionalIndexType>(maxLength: Int)(s: S) -> S.SubSlice {
    return suffix(s, maxLength)
}


/// Exchange the values of `a` and `b`
public func swap<T>(inout b: T)(inout a: T) {
    swap(&a, &b)
}
...
///    anything.
///
public func unsafeBitCast<T, U>(type: U.Type)(x: T) -> U {
...
public func withExtendedLifetime<T, Result>(@noescape f: () -> Result)(x: T) -> Result {
...
/// destroyed before f returns.
public func withExtendedLifetime<T, Result>(@noescape f: (T) -> Result)(x: T) -> Result {
    return withExtendedLifetime(x, f)
}
...
public func withUnsafeMutablePointer<T, Result>(@noescape body: (UnsafeMutablePointer<T>) -> Result)(inout arg: T) -> Result {
...
/// and `arg2`.
public func withUnsafeMutablePointers<A0, A1, A2, Result>(inout arg1: A1, inout arg2: A2, @noescape body: (UnsafeMutablePointer<A0>, UnsafeMutablePointer<A1>, UnsafeMutablePointer<A2>) -> Result)(inout arg0: A0) -> Result {
    return withUnsafeMutablePointers(&arg0, &arg1, &arg2, body)
}


/// Like `withUnsafeMutablePointer`, but passes pointers to `arg0` and `arg1`.
public func withUnsafeMutablePointers<A0, A1, Result>(inout arg1: A1, @noescape body: (UnsafeMutablePointer<A0>, UnsafeMutablePointer<A1>) -> Result)(inout arg0: A0) -> Result {
...
/// result. Useful for calling Objective-C APIs that take "in/out"
/// parameters (and default-constructible "out" parameters) by pointer
public func withUnsafePointer<T, Result>(@noescape body: (UnsafePointer<T>) -> Result)(inout arg: T) -> Result {
    return withUnsafePointer(&arg, body)
}


/// Like `withUnsafePointer`, but passes pointers to `arg0` and `arg1`.
public func withUnsafePointers<A0, A1, Result>(inout arg1: A1, @noescape body: (UnsafePointer<A0>, UnsafePointer<A1>) -> Result)(inout arg0: A0) -> Result {
...
/// and `arg2`.
public func withUnsafePointers<A0, A1, A2, Result>(inout arg1: A1, inout arg2: A2, @noescape body: (UnsafePointer<A0>, UnsafePointer<A1>, UnsafePointer<A2>) -> Result)(inout arg0: A0) -> Result {
    return withUnsafePointers(&arg0, &arg1, &arg2, body)
}


/// Invoke `f` with a C `va_list` argument derived from `builder`.
public func withVaList<R>(@noescape f: (CVaListPointer) -> R)(builder: VaListBuilder) -> R {
    return withVaList(builder, f)
}


/// Invoke `f` with a C `va_list` argument derived from `args`.
public func withVaList<R>(@noescape f: (CVaListPointer) -> R)(args: [CVarArgType]) -> R {
...
/// underlying sequence.
public func zip<S0 : SequenceType, S1 : SequenceType>(s1: S1)(s0: S0) -> Zip2<S0, S1> {
    return zip(s0, s1)
}
